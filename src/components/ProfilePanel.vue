...
const emit = defineEmits(['add', 'edit', 'delete', 'deleteAll', 'toggle', 'copyLink', 'batchGenerate']);
...
const handleBatchGenerate = (profileId) => emit('batchGenerate', profileId);
  ...
  <div>
    ...
    <div v-if="profiles.length > 0" class="space-y-4" style="z-index: 1; position: relative;">
      <ProfileCard
        ...
        @batch-generate="handleBatchGenerate(profile.id)"
      />
    </div>
